...
export function ProfileSettings() {
  ...
  useEffect(() => {
    setBreadcrumbs([
      { label: t("ui.settings"), href: "/company/settings" },
      { label: t("ui.profile") },
    ]);
  }, [setBreadcrumbs]);
  ...
  const updateMutation = useMutation({
    ...
    onError: (error) => {
      setActionError(error instanceof Error ? error.message : t("ui.profile_settings_update_error"));
    },
  });
  ...
  const uploadAvatarMutation = useMutation({
    mutationFn: async (file: File) => {
      if (!selectedCompanyId) {
        throw new Error(t("ui.profile_settings_select_organization_error"));
      }
    ...
    onError: (error) => {
      setActionError(error instanceof Error ? error.message : t("ui.profile_settings_upload_error"));
    },
  });
  ...
  const removeAvatarMutation = useMutation({
    ...
    onError: (error) => {
      setActionError(error instanceof Error ? error.message : t("ui.profile_settings_remove_error"));
    },
  });

  if (sessionQuery.isLoading) {
    return <div className="text-sm text-muted-foreground">{t("ui.profile_settings_loading")}</div>;
  }

  if (sessionQuery.error || !sessionQuery.data) {
    return (
      <div className="text-sm text-destructive">
        {sessionQuery.error instanceof Error ? sessionQuery.error.message : t("ui.profile_settings_load_error")}
      </div>
    );
  }

  const currentName = name.trim() || sessionQuery.data.user.name || t("ui.board");
  ...
  const uploadHint = selectedCompany
    ? t("ui.profile_settings_storage_hint", { company: selectedCompany.name })
    : t("ui.profile_settings_select_organization_hint");
    ...
    <div className="max-w-6xl space-y-6">
      <div className="space-y-2">
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-lg font-semibold">{t("ui.profile")}</h1>
        </div>
        <p className="text-sm text-muted-foreground">
          {t("ui.profile_settings_description")}
        </p>
      </div>
      ...
      <section className="space-y-8">
            ...
            <div className="flex flex-wrap items-end gap-5 rounded-(--rad-24) border border-border/70 bg-background/92 p-5 shadow-(--shadow-extract-18) backdrop-blur-sm">
              <div className="space-y-3">
                ...
                >
                  <input
                    ...
                    id={avatarInputId}
                    type="file"
                    aria-label={t("ui.profile_settings_upload_photo")}
                    accept="image/*"
                    className="sr-only"
                ...
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  >
                    {uploadAvatarMutation.isPending ? <LoaderCircle className="size-4 animate-spin" /> : <Camera className="size-4" />}
                    {currentImage ? t("ui.profile_settings_change_photo") : t("ui.profile_settings_upload_photo")}
                  </Button>
                  {currentImage ? (
                    <Button
                      type="button"
              ...
              <div className="min-w-0 flex-1 space-y-2 pb-1">
                <div>
                  ...
                  <p className="truncate text-sm text-muted-foreground">{sessionQuery.data.user.email ?? t("ui.profile_settings_no_email")}</p>
                </div>
                <p className="max-w-2xl text-sm leading-6 text-muted-foreground">
                  {t("ui.profile_settings_upload_hint")} {uploadHint}
                </p>
              </div>
            </div>
          </div>
        ...
        >
          <div className="space-y-2">
            <Label htmlFor="profile-name">{t("ui.display_name")}</Label>
            <Input
              id="profile-name"
              value={name}
              onChange={(event) => setName(event.target.value)}
              maxLength={120}
              placeholder={t("ui.board")}
            />
            <p className="text-xs text-muted-foreground">
              {t("ui.profile_settings_name_hint")}
            </p>
          </div>

          <div className="space-y-2">
            <Label htmlFor="profile-email">{t("ui.email")}</Label>
            <Input
              id="profile-email"
            ...
            />
            <p className="text-xs text-muted-foreground">
              {t("ui.profile_settings_email_hint")}
            </p>
          </div>
          ...
          <div className="md:col-span-2 flex justify-end">
            <Button type="submit" disabled={isSavingProfile || !name.trim()}>
              ...
              {updateMutation.isPending ? t("ui.saving") : t("ui.profile_settings_save")}
            </Button>
          </div>
        </form>
