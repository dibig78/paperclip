      ...
      <PopoverTrigger asChild>
        ...
        >
          {unknownManager ? (
            ...
            <>
              <AgentIcon agentId={current.id} agentName={current.name} icon={current.icon} className="h-3 w-3 shrink-0 text-muted-foreground" />
              <span
                className={cn(
                  "min-w-0 truncate",
      ...
      <PopoverContent className="w-48 p-1" align="start">
        ...
        {terminatedManager && (
          <div className="flex min-w-0 items-center gap-2 overflow-hidden px-2 py-1.5 text-xs text-muted-foreground border-b border-border mb-0.5">
            <AgentIcon agentId={current.id} agentName={current.name} icon={current.icon} className="shrink-0 h-3 w-3" />
            <span className="min-w-0 truncate">
        ...
        {rows.map((a) => (
          ...
          >
            <AgentIcon agentId={a.id} agentName={a.name} icon={a.icon} className="shrink-0 h-3 w-3 text-muted-foreground" />
